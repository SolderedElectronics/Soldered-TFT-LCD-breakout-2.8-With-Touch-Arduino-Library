...
public:
  ...
  // Custom constructor added by Soldered to use with 2.8" LCD breakout
  // It uses hardware SPI to send data to shift registers so it's in parallel mode
  // So we need Hardware SPI but also other pins used in parallel
  // Uses default SPI port
  Adafruit_SPITFT(uint16_t w, uint16_t h, SPIClass *spiClass, int8_t cs, int8_t dc,
                  int8_t wr, int8_t rd, int8_t rst);
...
protected:
  ...
  inline void TFT_CS_STROBE(void); // Parallel interface cs strobe, by Soldered
  ...
  // in an attempt to get values to align and pack better in RAM.
    ...
    struct {                     //   Values specific to 8-bit parallel:
      ...
      bool wide = 1;           ///< If true, is 16-bit interface
